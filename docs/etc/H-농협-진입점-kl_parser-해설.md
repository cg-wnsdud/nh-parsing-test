...
| ⑧ | ~~에이전트 분리 배포~~ → **코드 제거** | **해소 (2026-08-25)** | 부를 상대(`/v1/kl-export` 서버)가 없고 실행 이력·시험도 없어 지웠다. 대신 본체를 3.11 에서 돌게 맞추는 쪽으로 방향을 정했다 |
...
> ⚠️ 단 이건 **로컬 개발 편의**다. 농협 배포에서는 경로만 연결할 수 없으므로
> `nh_parsing` 과 그 의존성을 실제로 반입해야 한다.
