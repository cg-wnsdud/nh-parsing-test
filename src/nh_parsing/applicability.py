...
'값이 없다'는 서로 다른 세 가지 사실이 뭉개진 것이다:
  ...
  (B) 미표시    — 표시 의무가 있는 항목이 광고 텍스트에서 발견되지 않았다는 **사실**이다.
                  위반인지, 위험도가 얼마인지는 여기서 판정하지 않는다 — 그건 규정과
                  대조해 최종 심의하는 하류(RAG/DB 엔진 + 담당자) 몫이다. 우리가 넘기는
                  건 "미표시"라는 관측이지 "위반"이라는 결론이 아니다(2026-08-03 정정 —
                  이전 버전은 이 필드를 "심의 지적사항"으로 단정해 하류 판정을 침범했다).
