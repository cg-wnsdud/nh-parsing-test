...
class Settings:
    ...
    # ── 수치 필드 크롭 재확인 (6-5) — 기본 off ────────────────────
    # 금리·심의필 수치를 고해상 크롭으로 다시 읽혀 교정하던 단계. 문서당 최대 8회.
    # 끈 근거(2026-07-28):
    #   ① 3회 실행 연속 값 교체 0건 (26개 재판독 → 전원 원값 유지, extractor=vlm+crop 0개)
    #   ② 영역 통독(§6)이 같은 영역을 이미 VLM 으로 읽는다 — 이 단계는 통독 도입 전 설계였다
    #   ③ A/B 에서 전체 1537s → 1346s (12% 단축). 002 는 395s → 268s
    # 품질 A/B 는 결론이 안 났다(실행 간 변동이 더 컸고, 차이 난 항목이 전부 수치와 무관한
    # 헤드라인이었다). 다만 '교체 0건'은 정의상 출력에 영향이 없다는 뜻이라 끄기로 했다.
    # 코드는 남긴다 — 다른 데이터에서 필요해지면 NUMERIC_CROP_VERIFY=1 로 되살린다.
    numeric_crop_verify_enabled: bool = (
        os.environ.get("NUMERIC_CROP_VERIFY", "0") not in ("0", "false", "False")
    )
    ...
    # ── 영역별 VLM 통독 (§6, 플래그로 on/off) ──────────────────────────
    # StructureV3 영역 크롭을 VLM 이 통독해 텍스트 1차 출처로 삼고 OCR 은 강등한다.
    # 호출 수를 줄이려 여러 영역 크롭을 한 호출에 배치로 묶는다(HyundaiHS x4 방식).
    # 개수 상한은 두지 않는다 — 호출 수 = ceil(영역수 / crops_per_call).
    region_vlm_crops_per_call: int = 4         # 영역 통독 배치 크기 (호출당 크롭 장수)
    # 배치 통독은 여러 이미지를 한 프롬프트에 넣어 LVLM 의 cross-image leakage/sequence
    # confusion(크롭↔내용 뒤섞임)에 취약하다(문헌: arXiv 2508.13744 등, 올원e 상품안내표
    # r005~r007 오정렬 실측). → 통독 결과가 그 영역 OCR 원문과 토큰 정합(ocr_score)이
    # 임계 미만이면 그 영역만 단일 크롭으로 재통독하고 더 정합한 쪽을 채택한다.
    region_vlm_reread_min_score: float = 0.5   # 이 미만이면 단일 크롭 재통독 트리거
    region_vlm_reread_max_per_page: int = 16   # 페이지당 단일 재통독 호출 상한 (비용 가드)
    # 통독이 "정밀도 높음(모든 토큰이 OCR 에 있음) + 커버리지 낮음(OCR 내용을 많이 빠뜨림)"
    # 이면 OCR 의 순부분집합 = 순수 절단(값 누락)이므로 승격하지 않고 OCR 라인을 유지한다.
    # 회전/장식 교정은 OCR 에 없던 새 토큰이 있어 정밀도가 낮으므로 이 규칙에 안 걸린다.
    region_vlm_truncation_precision: float = 0.9  # 이 이상 정밀도 + 저커버리지 = 절단 판정

    # ── 밴드 단위 통합 판독 (④+ 통독 + ⑧ 스윕을 한 호출로) — 기본 on ────
    # OCR 이 본 것과 **같은 밴드**를 VLM 에도 주고, 그 안의 영역 목록을 함께 실어
    # "이 영역들을 고쳐라 + 목록에 없는 문구를 찾아라"를 한 호출로 묻는다.
    # 예전에는 ④+ 가 영역을 하나씩(4장 배치) 보고 스윕이 밴드를 따로 봐서, 같은 페이지를
    # 서로 다른 크롭으로 두 번 훑었다.
    #
    ...
    # 0 으로 두면 예전 경로(영역별 ④+ + 밴드 스윕)로 되돌아간다.
    merged_band_read: bool = (
        os.environ.get("MERGED_BAND_READ", "1") not in ("0", "false", "False")
    )
