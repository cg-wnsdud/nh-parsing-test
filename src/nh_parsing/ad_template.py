...
**공백에 기대지 않는다.** 현행 파서는 디지털 텍스트의 공백을 버린다
(`triage.py` 의 `if not ch or ch.isspace(): continue` — 2026-08-24 시점 미수정 결정).
그래서 `기타 자세한 내용은` 이 `기타자세한내용은` 으로 나온다. 대조는 양쪽에서
공백을 모두 지우고 한다 — 파서를 고치면 그때는 그냥 더 정확해질 뿐 깨지지 않는다.
"""
