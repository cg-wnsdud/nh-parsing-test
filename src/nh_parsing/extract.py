...
8. 섹션 제목에 "예시·목업 화면"이라고 붙은 곳은 앱 화면 목업·견본 이미지입니다. 거기 적힌
   금액·계좌번호·날짜는 실제 상품 조건이 아니므로 **값으로 쓰지 마세요**(예: 목업 화면의
   '100,000원 송금'을 금액 필드에 넣으면 안 됩니다). 다만 그 안에도 실제 심의 대상 문구가
   섞여 있을 수 있으니(헤드라인·이벤트 기간 등) 읽기는 하고, 값으로 쓸 때는 그것이 광고의
   실제 조건인지 화면 견본인지 판단해서 쓰세요. 애매하면 unmapped 에 남기세요."""
